...
class Smarty_Compiler extends Smarty {
    ...
    function _compile_file($resource_name, $source_content, &$compiled_content)
    {
            ...
            if (preg_match_all('~(<\?(?:\w+|=)?|\?>|language\s*=\s*[\"\']?\s*php\s*[\"\']?)~is', $text_blocks[$curr_tb], $sp_match)) {
                ...
                for ($curr_sp = 0, $for_max2 = count($sp_match[1]); $curr_sp < $for_max2; $curr_sp++) {
                    if ($this->php_handling == SMARTY_PHP_PASSTHRU) {
                        /* echo php contents */
                        $text_blocks[$curr_tb] = str_replace('%%%SMARTYSP'.$curr_sp.'%%%', '<?hh echo \''.str_replace("'", "\'", $sp_match[1][$curr_sp]).'\'; ?>'."\n", $text_blocks[$curr_tb]);
                        ...
                        $sp_match[1][$curr_sp] = preg_replace('~(<\?(?!php|=|$))~i', '<?hh echo \'\\1\'?>'."\n", $sp_match[1][$curr_sp]);
        ...
        for ($i = 0, $for_max = count($compiled_tags); $i < $for_max; $i++) {
            ...
            if ($strip) {
                ...
                for ($j = $i + 1; $j < $for_max; $j++) {
                    ...
                    $text_blocks[$j] = "<?hh echo '" . strtr($text_blocks[$j], array("'"=>"\'", "\\"=>"\\\\")) . "'; ?>";
                    if ($compiled_tags[$j] == '{/strip}') {
        ...
        $compiled_content = str_replace('<?', "<?hh echo '<?' ?>\n", $compiled_content);
        $compiled_content = preg_replace("~(?<!')language\s*=\s*[\"\']?\s*php\s*[\"\']?~", "<?hh echo 'language=php' ?>\n", $compiled_content);
        ...
        if (!empty($this->_cache_serial)) {
            $compiled_content = "<?hh \$this->_cache_serials['".$this->_cache_include."'] = '".$this->_cache_serial."'; ?>" . $compiled_content;
        }
        ...
        $template_header = "<?hh /* Smarty version ".$this->_version.", created on ".strftime("%Y-%m-%d %H:%M:%S")."\n";
        ...
        if (count($this->_plugin_info)) {
            ...
            $_plugins_params .= '))';
            $plugins_code = "<?hh require_once(SMARTY_CORE_DIR . 'core.load_plugins.php');\nsmarty_core_load_plugins($_plugins_params, \$this); ?>\n";
        ...
        if ($this->_init_smarty_vars) {
            $template_header .= "<?hh require_once(SMARTY_CORE_DIR . 'core.assign_smarty_interface.php');\nsmarty_core_assign_smarty_interface(null, \$this); ?>\n";
    ...
    function _compile_tag($template_tag)
    {
        ...
        if (preg_match('~^' . $this->_num_const_regexp . '|' . $this->_obj_call_regexp . '|' . $this->_var_regexp . '$~', $tag_command)) {
            ...
            return "<?hh echo $_return; ?>" . $this->_additional_newline;
        }
        ...
        switch ($tag_command) {
            ...
            case 'foreachelse':
                ...
                return "<?hh endforeach; else: ?>";

            case '/foreach':
                $_open_tag = $this->_pop_tag('foreach');
                if ($_open_tag == 'foreachelse')
                    return "<?hh endif; unset(\$_from); ?>";
                else
                    return "<?hh endforeach; endif; unset(\$_from); ?>";
                break;
            ...
            case 'php':
                ...
                switch (count($block)) {
                    ...
                    case 3: /* literal */
                        return "<?hh echo '" . strtr($block[2], array("'"=>"\'", "\\"=>"\\\\")) . "'; ?>" . $this->_additional_newline;
    ...
    function _compile_compiler_tag($tag_command, $tag_args, &$output)
    {
        ...
        if ($found) {
            if ($have_function) {
                ...
                if($output != '') {
                $output = '<?hh ' . $this->_push_cacheable_state('compiler', $tag_command)
                                   . $output
    ...
    function _compile_block_tag($tag_command, $tag_args, $tag_modifier, &$output)
    {
        ...
        if ($start_tag) {
            $output = '<?hh ' . $this->_push_cacheable_state('block', $tag_command);
            ...
            $output = '<?hh $_block_content = ob_get_contents(); ob_end_clean(); ';
    ...
    function _compile_custom_tag($tag_command, $tag_args, $tag_modifier, &$output)
    {
        ...
        if($output != '') {
            $output =  '<?hh ' . $_cacheable_state . $_cache_attrs . 'echo ' . $output . ';'
    ...
    function _compile_registered_object_tag($tag_command, $attrs, $tag_modifier)
    {
        ...
        return '<?hh ' . $prefix . $output . $postfix . "?>" . $newline;
    }
    ...
    function _compile_insert_tag($tag_args)
    {
        ...
        return "<?hh require_once(SMARTY_CORE_DIR . 'core.run_insert_handler.php');\necho smarty_core_run_insert_handler($_params, \$this); ?>" . $this->_additional_newline;
    }
    ...
    function _compile_include_php_tag($tag_args)
    {
        ...
        return "<?hh require_once(SMARTY_CORE_DIR . 'core.smarty_include_php.php');\nsmarty_core_smarty_include_php($_params, \$this); ?>" . $this->_additional_newline;
    }
    ...
    function _compile_capture_tag($start, $tag_args = '')
    {
        ...
        if ($start) {
            ...
            $output = "<?hh \$this->_smarty_vars['capture'][$buffer] = ob_get_contents(); ";
            if (isset($assign)) {
    ...
    function _compile_if_tag($tag_args, $elseif = false)
    {
        ...
        if ($elseif)
            return '<?hh elseif ('.implode(' ', $tokens).'): ?>';
        else
            return '<?hh if ('.implode(' ', $tokens).'): ?>';
    }
